...
export class WoumComponent implements OnChanges {
  ...
  useCarouselMedia = async (id: string, index: number) => {
    const a = await this.trackingManagerService.downloadFileByUrl(id);
    console.log(a);
    // this.testImgUrl[index].push(a);
    // console.log(this.testImgUrl[index]);
    ...
    this.testImgUrl[index] = a;
    let reader = new FileReader();
    reader.addEventListener("load", () => {
      this.testImgUrl[index] = reader.result;
    }, false);
    if (this.testImgUrl[index]) {
      reader.readAsDataURL(this.testImgUrl[index]);
      this.testCarouselImages.push(this.sanitizer.bypassSecurityTrustUrl(URL.createObjectURL(this.testImgUrl[index])));

    }
    console.log(this.testCarouselImages);
