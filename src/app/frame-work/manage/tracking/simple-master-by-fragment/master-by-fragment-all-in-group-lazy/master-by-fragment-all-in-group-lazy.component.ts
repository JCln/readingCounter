...
@Component({
  selector: 'app-master-by-fragment-all-in-group-lazy',
  templateUrl: './master-by-fragment-all-in-group-lazy.component.html',
  styleUrls: ['./master-by-fragment-all-in-group-lazy.component.scss']
})
export class MasterByFragmentAllInGroupLazyComponent extends AllListsFactory implements AfterViewInit {
  ...
  _selectedColumnsToRemember: string = 'selectedMasterByFragmentAllInGroupLazy';
  _sessionName: string = 'listMasterByFragmentAllInGroupLazy';
  _outputFileName: string = 'listMasterByFragmentAllInGroupLazy';
  _selectCols: any = [];
  ...
  updateOnChangedCounterState = async (event: any) => {
    if (MathS.isNull(this.pageSignsService.masterByFragmentAllInGroupLazy_pageSign.GUid))
      return;
    ...
    this.counterStateByZoneDictionary = await this.listManagerService.dictionaryWrapperService.getCounterStateByZoneIdDictionary(this.pageSignsService.masterByFragmentAllInGroupLazy_pageSign.zoneId);
    this.qotrDictionary = await this.listManagerService.dictionaryWrapperService.getQotrDictionary();
    this.resetDataSourceView();


    this.closeTabService.masterByFragmentAllInGroupLazy.data =
    ...
    Converter.convertIdToTitle(this.closeTabService.masterByFragmentAllInGroupLazy.data, this.karbariDictionaryCode, 'karbariCode');
    this.listManagerService.setDynamicPartRanges(this.closeTabService.masterByFragmentAllInGroupLazy.data);
  }
  classWrapper = async (canRefresh?: boolean) => {
    console.log(this.pageSignsService.masterByFragmentAllInGroupLazy_pageSign);

    if (!this.pageSignsService.masterByFragmentAllInGroupLazy_pageSign.GUid) {
  ...
  LazyLoading(event: LazyLoadEvent) {
    ...
    if (event.filters.hasOwnProperty('counterStateId'))
      event.filters['counterStateId'][0].value = this.closeTabService.masterByFragmentallInGroupLazyReq.multiSelectCounterStateId.length > 0 ? this.closeTabService.masterByFragmentallInGroupLazyReq.multiSelectCounterStateId : null;
    if (event.filters.hasOwnProperty('preCounterStateCode'))
      event.filters['preCounterStateCode'][0].value = this.closeTabService.masterByFragmentallInGroupLazyReq.multiSelectPreCounterStateCode.length > 0 ? this.closeTabService.masterByFragmentallInGroupLazyReq.multiSelectPreCounterStateCode : null;
    if (event.filters.hasOwnProperty('karbariCode'))
      event.filters['karbariCode'][0].value = this.closeTabService.masterByFragmentallInGroupLazyReq.multiSelectkarbariCode.length > 0 ? this.closeTabService.masterByFragmentallInGroupLazyReq.multiSelectkarbariCode : null;
    if (event.filters.hasOwnProperty('hazf'))
      event.filters['hazf'][0].value = this.closeTabService.masterByFragmentallInGroupLazyReq.multiSelectHazf.length > 0 ? this.closeTabService.masterByFragmentallInGroupLazyReq.multiSelectHazf : null;
    if (event.filters.hasOwnProperty('masrafStateId'))
      event.filters['masrafStateId'][0].value = this.closeTabService.masterByFragmentallInGroupLazyReq.multiSelectMasrafStateId.length > 0 ? this.closeTabService.masterByFragmentallInGroupLazyReq.multiSelectMasrafStateId : null;

    this.updateOnChangedCounterState(event);
  }
  changedFilterDropdowns(eventValue: any, elementName: string) {
    this.closeTabService.masterByFragmentallInGroupLazyReq[elementName] = eventValue;
  }
  // have problem on SHOWING Without this Code for DropDowns
  clickedDropDowns = (event: any, element: string, dataId: any) => {
    for (let index = 0; index < this.closeTabService.masterByFragmentAllInGroupLazy.data.length; index++) {
      if (this.closeTabService.masterByFragmentAllInGroupLazy.data[index].id === dataId) {
        this.closeTabService.masterByFragmentAllInGroupLazy.data[index][element] = event.title;
      }
    }
  }
  ...
  receiveDateJalali = (event: any, dataId: string) => {
    ...
    for (let index = 0; index < this.closeTabService.masterByFragmentAllInGroupLazy.data.length; index++) {
      if (this.closeTabService.masterByFragmentAllInGroupLazy.data[index].id === dataId) {
        this.closeTabService.masterByFragmentAllInGroupLazy.data[index].offloadDateJalali = event;
      }
    }
  }
  ...
  getExcel = async () => {
    const res = await this.listManagerService.ajaxReqWrapperService.getBlobByIdAsJson(ENInterfaces.GeneralModifyAllExcelInGroup, this.pageSignsService.masterByFragmentAllInGroupLazy_pageSign.groupId);
  ...
  clearFilters(table: Table) {
    ...
    this.closeTabService.masterByFragmentallInGroupLazyReq.multiSelectPreCounterStateCode = [];
    this.closeTabService.masterByFragmentallInGroupLazyReq.multiSelectkarbariCode = [];
    this.closeTabService.masterByFragmentallInGroupLazyReq.multiSelectHazf = [];
    this.closeTabService.masterByFragmentallInGroupLazyReq.multiSelectMasrafStateId = [];
