...
@Component({
  ...
  templateUrl: './request-draft-getlazy.component.html',
  styleUrls: ['./request-draft-getlazy.component.scss']
})
export class RequestDraftGetlazyComponent extends AllListsFactory implements AfterViewInit {
  ...
  clickedDropDowns = (event: any, element: string, dataId: any) => {
    for (let index = 0; index < this.closeTabService.requestDraftLazy.data.length; index++) {
      if (this.closeTabService.requestDraftLazy.data[index].id === dataId) {
        this.closeTabService.requestDraftLazy.data[index][element] = event.title;
      }
    }
  }
